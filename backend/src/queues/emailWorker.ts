...
import { EMAIL_QUEUE_NAME, EmailJobData, emailQueue } from "./emailQueue.js";
import {
  canSendEmail,
  incrementRateLimit,
  getNextHourKey,
...
const processEmailJob = async (job: Job<EmailJobData>) => {
  ...
  if (!allowed) {
    ...
    console.log(
      `⏳ Rate limit reached (${currentCount}/${limit}). Rescheduling job ${job.id} to next hour (${delayMs}ms delay)`
    );
  ...
  try {
    ...
    console.log(
      `✅ Email sent successfully: ${recipientEmail} (Preview: ${result.previewUrl})`
    );
