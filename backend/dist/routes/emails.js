...
router.post("/schedule", async (req, res) => {
    try {
        const { subject, body, recipients, scheduledAt, delayBetweenEmails } = req.body;
        // Validation
        ...
        console.log(`📧 Scheduling ${recipients.length} emails for batch ${batchId}`);
        const createdEmails = [];
        const scheduledJobs = [];
        for (let i = 0; i < recipients.length; i++) {
            ...
            const job = await scheduleEmail({
                ...
                userId,
                batchId,
            }, emailDelay, `email-${emailRecord.id}` // Idempotency key
            );
...
router.get("/scheduled", async (req, res) => {
    try {
        ...
        const scheduledEmails = await db.query.emails.findMany({
            where: and(eq(emails.userId, userId), eq(emails.status, "scheduled")),
...
router.get("/sent", async (req, res) => {
    try {
        ...
        const sentEmails = await db.query.emails.findMany({
            where: and(eq(emails.userId, userId), eq(emails.status, "sent")),
...
router.get("/failed", async (req, res) => {
    try {
        ...
        const failedEmails = await db.query.emails.findMany({
            where: and(eq(emails.userId, userId), eq(emails.status, "failed")),
